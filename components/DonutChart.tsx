...
const DonutChart: React.FC<{ budget: Budget, currency: Currency, onDeleteCategory: (category: string) => void, onEditCategory: (category: string) => void }> = ({ budget, currency, onDeleteCategory, onEditCategory }) => {
  ...
  if (totalSpent === 0 && Object.keys(budget).length === 0) {
      return <div className="text-center text-slate-500 dark:text-slate-400 p-8">No budget categories defined. Click 'Add Category' to start.</div>;
  }
  
  if (totalSpent === 0) {
      return <div className="text-center text-slate-500 dark:text-slate-400 p-8">No spending data to display yet.</div>;
  }
                ...
                {Object.entries(budget).map(([category, data]) => (
                    <li key={category} className="flex items-center justify-between text-sm">
                        ...
                        <div className="flex items-center gap-1">
                          <span className="font-semibold text-slate-800 dark:text-slate-200 mr-1">{formatCurrency(data.spent, currency.code, currency.locale, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span>
                           <button
